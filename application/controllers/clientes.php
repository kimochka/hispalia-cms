...
class Clientes extends CI_Controller {
	...
	function index() {
		...
		*/
		...
		foreach ($query->result() as $row) {
			...
			$this->db->where('cliente_id', $cID);
			$comprasCliente = $this->db->get('clientes_compran')->result_array();
			...
			if(empty($comprasCliente))
            	$actions .= '<span class="fa fa-lg fa-trash-o deleteButton" title="Borrar" id="button-delete-'. $cID .'"></span>';
		...
		$this->load->view("clientes/general_table", $this->data);
	...
	function nuevo() {
		...
		$provincias = Array();
        // $provincias[] = "-";
		// $provinciasObjectArray = $this->clientes_model->getProvinciasID($paisDefault);
		// foreach ($provinciasObjectArray as $row) {
			// $provincias[$row->provincia_id] = $row->nombre;
		// }
		$this->data['provincias'] = $provincias;
		
		
		$this->data['header_title'] = "Hispalia | Clientes - Nuevo";
		$this->data['h3'] = "CLIENTES";
		$this->data['breadcrumb'] = "Nuevo Cliente";
		$this->data['portlet_caption'] = "Completar el formulario del nuevo Cliente";
		...
		$this->data['autocompleteModifcarURL'] = "/clientes/modificar";
		
		$this->data['dialogAddNewLink'] = "nuevo";
		
		//Variables php que se cargaran en form-validation.js
		$this->data['buttonAddNew'] = "Añadir otro cliente nuevo";
		$this->data['buttonModifyCurrent'] = "Modificar este cliente";
		$this->data['buttonViewAll'] = "Ver tabla con todos los clientes";
		$this->data['buttonAddNewURL'] ="clientes/nuevo";
		$this->data['buttonModifyCurrentURL'] ="clientes/modificar/";
		$this->data['buttonControllerURL'] ="clientes";
	...
	function salvar_cliente() {
    		...
    		// echo "Valor del Delegado: ".$this->input->post("lista_delegados")."</br>";
    		
    		//cliente con Delegado
    		if ($this->input->post('group_name') == "Yes") {
    			$data_cliente['delegado_id'] = $this->input->post("lista_delegados");
    		}			
    		//cliente sin Delegado
    		else {
    			$data_cliente['delegado_id'] = NULL;
    		}
    		
    		$data_cliente['es_delegado'] = 0;
    		
    		
    		// //Se quiere introducir un nuevo cliente con delegado
    		// if ($this->input->post('group_name') == "Yes") {
    			// $query = $this->clientes_model->getDelegados();	
    			// //Comprueba si hay delegados en la BBDD.		
    			// if($query != -1) {
    				// //Buscar el numero de id del delegado.	
    				// $query2 = $this->clientes_model-> get_id_delegado($query[$this->input->post("lista_delegados")]);
    				// foreach ($query2->result() as $row) {
    					// $data_cliente['delegado_id'] = $row->cliente_id;
    				// }
    			// } else {
    				// //No hay delegados en la BBDD -> -1
    				// $data_cliente['delegado_id'] = -1;
    			// }
    		// }
    // 
    		// // Se quiere introducir un Cliente nuevo SIN delegado (= libre = delegado_id = NULL)
    		// else {
    			// $data_cliente['delegado_id'] = NULL;
    		// }
			...
			if ($this->input->post('action') != "modify") {
				...
				$data['dialogInfoResponse'] = "Cliente registrado.";
	...
	function modificar() {
		...
		$this->data['header_title'] = "Hispalia | Clientes - Modificar";

		$this->data['h3'] = "CLIENTES";
		$this->data['breadcrumb'] = "Modificar Cliente";
		$this->data['portlet_caption'] = "Completar el formulario del Cliente";
	...
	private function inzialize_js()
	{
		...
		// BEGIN PAGE LEVEL SCRIPTS	
			...
			base_url().CLIENTES,
			base_url().FORM_VALIDATION_CLIENTES,
			base_url().TABLE_GENERAL_CLIENTES
		);	
	}
